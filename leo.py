...
def shuffle():
        ...
        y = (display_height * .5) - 100
        for pile in range(10,display_width,160):
                if counter < 8:
                        
                        randomCard = random.randint(0,len(deck)-1)
                        selectedCard.append(randomCard)
                        piles.append(gameDisplay.blit(deck[randomCard], (pile, y)))
                        button("Moving", pile, y + 300, 150, 100, white, gray)
                        counter +=1          
        pygame.display.update()
                
        

def update(selected,mx,my):
        
        gameDisplay.blit(deck[selectedCard[selected]], (mx,my))
        pygame.display.update()      
...
#                         button("Refresh",700,0,100,100,white,white,shuffle)
...
def game():
    ...
    while not crashed:
        ...
        #                     click = True
        #                     piles[selected].x = event.pos[0] + selected_offset_x
        #                     piles[selected].y = event.pos[1] + selected_offset_y
        #                     mx,my = pygame.mouse.get_pos()

        # if(click is True and selected is not None):
        #         print(mx,my)
        #         print(selectedCard[selected])
        #         update(selected,mx,my)
        ...
        # when the player need refresh the cards from Draw Pile or restart the game
        # #########################################################################    
        button("Refresh", 900, 0, 150, 100, white, gray, shuffle)
